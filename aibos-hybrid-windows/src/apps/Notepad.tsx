...
export const Notepad = memo(() => {
  ...
  const loadNotes = async () => {
    try {
      const response = await fetch(`/api/notes?tenant=${tenantId}`);
      if (response.ok) {
  ...
  const loadQuota = async () => {
    try {
      const response = await fetch(`/api/quota?tenant=${tenantId}`);
      if (response.ok) {
        const data = await response.json();
        setQuota(data.quota);
      }
  ...
  const handleSave = async () => {
    ...
    try {
      const url = currentNoteId 
        ? `/api/notes/${currentNoteId}?tenant=${tenantId}`
        : `/api/saveNote?tenant=${tenantId}`;
  ...
  const loadNote = async (noteId: string) => {
    try {
      const response = await fetch(`/api/notes/${noteId}?tenant=${tenantId}`);
      if (response.ok) {
